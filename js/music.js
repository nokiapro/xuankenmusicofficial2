...
const GOOGLE_SHEET_API = 'https://script.google.com/macros/s/AKfycbzv1Em-q9iZyub39FP-b95wm59YcqTN9Dje5M_8cVhsf42g-TbUvqYSPHrBomGqBTc5hg/exec';
...
async function fetchSongsFromSheet() {
    try {
        ...
        const songsArray = Object.values(data).filter(song => song.id && song.id.trim() !== '');
        
        if (songsArray.length === 0) {
            console.error("LỖI: KHÔNG CÓ DỮ LIỆU BÀI HÁT TỪ GOOGLE SHEET!");
            return false;
        }
...
async function initApp() {
    ...
    if (songs.length === 0) {
        console.error("KHÔNG THỂ TẢI DỮ LIỆU BÀI HÁT!");
        adjustLyricFontSize("KHÔNG THỂ TẢI DANH SÁCH BÀI HÁT. VUI LÒNG THỬ LẠI SAU.");
        return;
    }
    
    initIdx = getInitialShuffleIndex();
    index = initIdx;
    ...
    if (songTitleEl) {
        songTitleEl.innerText = firstSong.name;
        applyGradientToSongTitle();
    }
    if (artistNameEl) {
        artistNameEl.innerText = 'XuanKen Official';
    ...
    audio.src = firstSong.audio1;
    audio.load();
    
    lyrics = await fetchLyricWithFallback(firstSong.lrc1, firstSong.lrc2);
    if (lyrics.length === 0) adjustLyricFontSize("BÀI HÁT TẠM CHƯA CÓ LYRIC NHA HIHI");
